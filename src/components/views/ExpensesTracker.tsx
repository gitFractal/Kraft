...
export default function ExpensesTracker() {
  const { convert, format, baseCurrency, displayCurrency, CURRENCY_SYMBOLS } = useCurrency();
  const [expenses, setExpenses] = useState([
    { id: '1', date: '2026-03-02', category: 'Groceries', amount: 120.50, currency: 'USD' },
    { id: '2', date: '2026-03-04', category: 'Utilities', amount: 145.00, currency: 'USD' },
    { id: '3', date: '2026-03-08', category: 'Dining Out', amount: 65.00, currency: 'USD' },
    { id: '4', date: '2026-03-10', category: 'Transportation', amount: 40.00, currency: 'USD' },
  ]);

  const [newExpense, setNewExpense] = useState({ date: '', category: '', amount: '', currency: displayCurrency });

  const totalExpense = expenses.reduce((sum, item) => sum + convert(item.amount, item.currency, displayCurrency), 0);
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    setExpenses([...expenses, { 
      ...
      currency: newExpense.currency || displayCurrency
    }]);
    setNewExpense({ date: '', category: '', amount: '', currency: displayCurrency });
  };
      ...
      <CardContent>
        <form onSubmit={handleAdd} className="flex flex-wrap gap-3 mb-6">
          ...
          <div className="flex w-full md:w-auto">
            ...
            <Input type="number" step="0.01" placeholder="Amount" value={newExpense.amount} onChange={e => setNewExpense({...newExpense, amount: e.target.value})} className="rounded-l-none w-full md:w-32" required />
          </div>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-100 dim:divide-gray-800 dark:divide-gray-800">
              {expenses.map((exp) => (
                <tr key={exp.id} className="hover:bg-gray-50 dim:hover:bg-[#3d3d3d] dark:hover:bg-[#242424] transition-colors">
                  ...
                  <td className="py-3 text-right text-gray-500">{format(exp.amount, exp.currency)}</td>
                  <td className="py-3 text-right font-bold text-red-600 dark:text-red-400">{format(convert(exp.amount, exp.currency, displayCurrency), displayCurrency)}</td>
            ...
            <tfoot>
              <tr className="border-t-2 border-gray-200 dim:border-gray-700 dark:border-gray-800">
                <td colSpan={3} className="py-4 font-bold text-right">Total Expenses:</td>
                <td className="py-4 text-right font-bold text-lg text-red-600 dark:text-red-400">{format(totalExpense, displayCurrency)}</td>
                <td></td>
              </tr>
            </tfoot>
          </table>
